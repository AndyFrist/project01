        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (null != mOnRefreshProcessListener && pullDownY > 0) {
                    ...
                    mOnRefreshProcessListener.onPull(refreshView, pullDownY, OnPullProcessListener.REFRESH);
                }
                ...
                if (mEvents == 0) {
                    if (pullDownY > 0 || (((Pullable) pullableView).canPullDown() && mCanPullDown && mPullDownEnable && state != LOADING)) {
                        // 可以下拉，正在加载时不能下拉
                        // 对实际滑动距离做缩小，造成用力拉的感觉
                    ...
                    } else if (pullUpY < 0 || (((Pullable) pullableView).canPullUp() && mCanPullUp && mPullUpEnable && state != REFRESHING)) {
                        // 可以上拉，正在刷新时不能上拉
    ...
    static class UpdateHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (null != layout) {
                ...
                layout.mMoveSpeed = (float) (8 + 5 * Math.tan(Math.PI / 2 / layout.getMeasuredHeight() * (layout.pullDownY + Math.abs(layout.pullUpY))));
                if (!layout.isTouch) {
                    // 正在刷新，且没有往上推的话则悬停，显示"正在刷新..."
                    if (layout.state == REFRESHING && layout.pullDownY <= layout.refreshDist) {
                        layout.pullDownY = layout.refreshDist;
                        layout.timer.cancel();
                    } else if (layout.state == LOADING && -layout.pullUpY <= layout.loadmoreDist) {
